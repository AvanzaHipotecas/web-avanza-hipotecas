...
import {
  ...
  Send, Phone, Mail, Star, FileText, TrendingUp, Globe, MapPin
} from 'lucide-react';
...
const HipotecaNoResidentesPage = () => {
  const [openFAQ, setOpenFAQ] = useState(null);
  const formRef = useRef();
  const { toast } = useToast();
  const [formData, setFormData] = useState({
    name: '', email: '', phone: '', propertyValue: '',
    mortgageType: 'no-residente', currentSituation: '', message: '', privacy: false, marketing: false,
  });
  const [isSubmitting, setIsSubmitting] = useState(false);

  const scrollToContact = (e) => {
    e?.preventDefault();
    document.getElementById('contacto-noresi')?.scrollIntoView({ behavior: 'smooth' });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const emailPromise = emailjs.send('YOUR_SERVICE_ID', 'YOUR_TEMPLATE_ID', {
        ...
        mortgage_type: 'Hipoteca No Residentes', current_situation: formData.currentSituation || 'No especificado',
      ...
      if (webhookRes.status === 'fulfilled' || emailRes.status === 'fulfilled') {
        ...
        setFormData({ name: '', email: '', phone: '', propertyValue: '', mortgageType: 'no-residente', currentSituation: '', message: '', privacy: false, marketing: false });
  ...
  const ventajas = [
    { icon: Percent, title: 'Financiación competitiva para no residentes', desc: 'Accede a la mayor financiación posible según tu perfil. Negociamos las mejores condiciones del mercado para compradores internacionales.' },
    { icon: Star, title: 'Comparamos más de 20 bancos', desc: 'Analizamos todas las entidades especializadas en hipotecas para no residentes y seleccionamos la más adecuada para tu perfil.' },
    { icon: Globe, title: 'Gestión completa a distancia', desc: 'Coordinamos todo el proceso sin que necesites estar físicamente en España. Tú decides cuándo venir a firmar.' },
    { icon: FileText, title: 'Orientación con el NIE y trámites previos', desc: 'Te guiamos en la obtención del NIE y todos los requisitos administrativos previos a la compra.' },
    { icon: MapPin, title: 'Conocemos el mercado inmobiliario español', desc: 'Te asesoramos también sobre zonas, precios y aspectos legales de la compra de vivienda en España.' },
    { icon: FileText, title: 'Estudio sin coste', desc: 'El estudio de tu caso es completamente gratuito y sin compromiso.' },
  ];

  const requisitos = [
    'NIE (Número de Identificación de Extranjero) o pasaporte para ciudadanos UE',
    'Ingresos demostrables en el país de residencia',
    'Historial crediticio limpio en tu país',
    'Declaración de impuestos del país de residencia (últimos 2 años)',
  ];

  const faqs = [
    {
      question: '¿Puedo solicitar una hipoteca en España si vivo en el extranjero?',
      answer: 'Sí, absolutamente. Tanto si eres extranjero como si eres español residente fuera de España, puedes solicitar una hipoteca para comprar una vivienda en España. El proceso tiene algunas particularidades respecto a una hipoteca para residentes, pero en Avanza Hipotecas conocemos perfectamente estos requisitos y te guiamos en cada paso.',
    },
    {
      question: '¿Qué porcentaje del valor financia el banco a un no residente?',
      answer: 'Los bancos suelen financiar entre el 60% y el 70% del valor de compraventa para no residentes, frente al 80% habitual para residentes. Esto significa que necesitarás disponer de ahorros para cubrir la diferencia más los gastos de compra. Trabajamos con las entidades que ofrecen las condiciones más favorables para perfiles no residentes.',
    },
    {
      question: '¿Necesito el NIE antes de iniciar el proceso hipotecario?',
      answer: 'El NIE es imprescindible para formalizar la compra y firmar la hipoteca, pero puedes iniciar el estudio sin él. De hecho, es recomendable comenzar el proceso hipotecario en paralelo a la solicitud del NIE para ganar tiempo. Te orientamos sobre cómo obtenerlo desde tu país de residencia o a través del consulado español.',
    },
    {
      question: '¿Puedo gestionar todo el proceso a distancia sin venir a España?',
      answer: 'La mayor parte del proceso sí puede gestionarse a distancia: estudio, análisis de tu perfil, negociación con bancos y preparación de documentación. La firma ante notario requiere presencia física en España o, en algunos casos, puede hacerse mediante poder notarial otorgado en tu país. Te explicamos todas las opciones.',
    },
  ];
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              ...
              >
                Compra tu vivienda en España{' '}
                <span className="text-[#2EBFA5]">desde cualquier parte del mundo</span>
              </motion.h1>
              <motion.p
                initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
                className="text-xl text-gray-600 mb-8 leading-relaxed"
              >
                Accede a financiación hipotecaria en España siendo no residente. Gestionamos todo el proceso en tu nombre.
              </motion.p>
              <motion.div
              ...
              >
                {['Estudio 100% gratuito', 'Gestión a distancia', '+20 bancos comparados'].map((item) => (
            ...
            >
              <div className="relative">
                <img src="https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=800&q=75&fm=webp" alt="Vista de costa española con viviendas" className="rounded-3xl shadow-2xl w-full object-cover h-[480px]" width="800" height="480" loading="lazy" />
                <div className="absolute -bottom-6 -left-6 bg-white p-4 rounded-2xl shadow-lg flex items-center space-x-3">
                  ...
                  <div>
                    <p className="font-bold text-[#1A3C40]">Gestión a distancia</p>
                    <p className="text-sm text-gray-600">Sin venir a España</p>
                  </div>
                </div>
              </div>
            </motion.div>
      ...
      <section className="py-20 bg-[#1A3C40] text-white">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
            ...
            <p className="text-xl text-gray-300 max-w-2xl mx-auto">Para cualquier persona que quiera comprar una vivienda en España sin residir en el país</p>
          </motion.div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
            {[
              { icon: Globe, title: 'Extranjeros en España', desc: 'Ciudadanos de cualquier país que quieren comprar su vivienda principal o segunda residencia en España' },
              { icon: Users, title: 'Españoles en el extranjero', desc: 'Ciudadanos españoles residentes fuera de España que quieren comprar aquí' },
              { icon: TrendingUp, title: 'Inversores internacionales', desc: 'Compradores que buscan invertir en el mercado inmobiliario español' },
              { icon: Home, title: 'Segunda residencia en España', desc: 'Compradores que quieren una vivienda vacacional o de retiro en España' },
            ].map((item, i) => (
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
            <h2 className="text-4xl md:text-5xl font-bold text-[#1A3C40] mb-4">Ventajas de nuestra <span className="text-[#2EBFA5]">Hipoteca No Residentes</span></h2>
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">Tu aliado en España para que la distancia no sea un obstáculo</p>
          </motion.div>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
              ...
              <p className="text-xl text-gray-600">Documentación habitual para solicitar una hipoteca como no residente en España</p>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-10 bg-[#2EBFA5]/10 border border-[#2EBFA5]/30 rounded-2xl p-6 text-center">
              <p className="text-[#1A3C40] font-semibold text-lg">¿Tienes dudas sobre tu documentación?</p>
              <p className="text-gray-600 mt-2 mb-4">Cuéntanos tu situación y te explicamos exactamente qué necesitas para tu caso concreto.</p>
      ...
      {/* ── FORMULARIO ── */}
      <section id="contacto-noresi" className="py-20 bg-[#E9ECEF]">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
            ...
            <p className="text-xl text-gray-700 max-w-3xl mx-auto">Completa el formulario y un experto en hipotecas para no residentes te contactará en menos de 24 horas</p>
          </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-white rounded-2xl p-8 shadow-sm">
              ...
              <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label htmlFor="phone" className="text-gray-700 font-medium">Teléfono (con prefijo) *</Label>
                    <Input id="phone" name="phone" type="tel" required value={formData.phone} onChange={handleInputChange} className="mt-2" placeholder="+44 7700 123456" />
                  </div>
                </div>
                <div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label htmlFor="currentSituation" className="text-gray-700 font-medium">Situación</Label>
                    <Select id="currentSituation" name="currentSituation" value={formData.currentSituation} onChange={handleInputChange} className="mt-2">
                      ...
                      <option value="extranjero-ue">Extranjero ciudadano UE</option>
                      <option value="extranjero-no-ue">Extranjero fuera de la UE</option>
                      <option value="espanol-extranjero">Español residente en el extranjero</option>
                    </Select>
                  </div>
                </div>
                <div>
                  <Label htmlFor="message" className="text-gray-700 font-medium">Cuéntanos tu caso</Label>
                  <Textarea id="message" name="message" value={formData.message} onChange={handleInputChange} className="mt-2" placeholder="País de residencia, zona de España donde buscas, si ya tienes el NIE, uso previsto (principal, vacacional, inversión)..." />
                </div>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="space-y-8">
              <div className="bg-[#1A3C40] text-white rounded-2xl p-8">
                ...
                <div className="space-y-6">
                  <div className="flex items-center"><Phone className="w-6 h-6 mr-4 text-[#2EBFA5]" /><div><div className="font-semibold">Teléfono de contacto</div><div className="text-gray-300">+34 624 810 190</div></div></div>
                  ...
                  <div className="flex items-start"><Clock className="w-6 h-6 mr-4 text-[#2EBFA5] mt-1 flex-shrink-0" /><div><div className="font-semibold">Horario de atención (hora española)</div><div className="text-gray-300">Lunes a Jueves: 9:00h - 14:00h y 16:00h - 19:00h</div><div className="text-gray-300">Viernes: 8:00h - 14:00h</div></div></div>
                </div>
              </div>
              <div className="bg-white rounded-2xl p-8 shadow-sm">
                ...
                <div className="space-y-4">
                  {[
                    { n: 1, title: 'Análisis de tu perfil internacional', desc: 'Estudiamos tu país de residencia, ingresos y documentación disponible' },
                    { n: 2, title: 'Búsqueda entre +20 bancos', desc: 'Seleccionamos las entidades con mejores condiciones para no residentes' },
                    { n: 3, title: 'Presentamos tu propuesta', desc: 'Recibes las mejores ofertas para tu caso, sin necesidad de venir a España' },
                    { n: 4, title: 'Coordinamos la firma', desc: 'Organizamos toda la documentación y la fecha de firma ante notario' },
                  ].map(({ n, title, desc }) => (
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <Link to="/hipoteca-100" className="bg-white rounded-2xl p-5 flex items-start gap-4 hover:shadow-md transition-shadow border border-gray-100 hover:border-[#2EBFA5]/40 group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">¿Quieres maximizar la financiación? Descubre cómo conseguir el 100% del precio sin necesidad de entrada</p>
                </div>
                <ArrowRight className="w-4 h-4 text-gray-400 flex-shrink-0 group-hover:text-[#2EBFA5] transition-colors" />
              </Link>
              <Link to="/hipoteca-reforma" className="bg-white rounded-2xl p-5 flex items-start gap-4 hover:shadow-md transition-shadow border border-gray-100 hover:border-[#2EBFA5]/40 group">
                <div className="w-10 h-10 bg-[#2EBFA5]/10 rounded-xl flex items-center justify-center flex-shrink-0">
                  <FileText className="w-5 h-5 text-[#2EBFA5]" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-[#1A3C40] group-hover:text-[#2EBFA5] transition-colors text-sm">Hipoteca Reforma</p>
                  <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">¿La vivienda que compras necesita obras? Financiamos también el coste de la reforma en la misma hipoteca</p>
                </div>
                <ArrowRight className="w-4 h-4 text-gray-400 flex-shrink-0 group-hover:text-[#2EBFA5] transition-colors" />
              </Link>
            </div>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
            ...
            <p className="text-xl text-gray-600">Todo lo que necesitas saber para comprar una vivienda en España desde el extranjero</p>
          </motion.div>
          <div className="max-w-4xl mx-auto">
            {faqs.map((faq, i) => (
...
export default HipotecaNoResidentesPage;
