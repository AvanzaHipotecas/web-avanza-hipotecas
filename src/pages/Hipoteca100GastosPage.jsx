...
import {
  ...
  Send, Phone, Mail, Star, FileText, TrendingUp, Wallet
} from 'lucide-react';
...
const Hipoteca100GastosPage = () => {
  const [openFAQ, setOpenFAQ] = useState(null);
  const formRef = useRef();
  const { toast } = useToast();
  const [formData, setFormData] = useState({
    name: '', email: '', phone: '', propertyValue: '',
    mortgageType: '100-gastos', currentSituation: '', message: '', privacy: false, marketing: false,
  });
  const [isSubmitting, setIsSubmitting] = useState(false);

  const scrollToContact = (e) => {
    e?.preventDefault();
    document.getElementById('contacto-h100g')?.scrollIntoView({ behavior: 'smooth' });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const emailPromise = emailjs.send('YOUR_SERVICE_ID', 'YOUR_TEMPLATE_ID', {
        ...
        mortgage_type: 'Hipoteca 100% + Gastos', current_situation: formData.currentSituation || 'No especificado',
      ...
      if (webhookRes.status === 'fulfilled' || emailRes.status === 'fulfilled') {
        ...
        setFormData({ name: '', email: '', phone: '', propertyValue: '', mortgageType: '100-gastos', currentSituation: '', message: '', privacy: false, marketing: false });
  ...
  const ventajas = [
    { icon: Wallet, title: 'Precio + gastos en un solo préstamo', desc: 'Cubre el 100% del precio de compraventa más notaría, registro e impuestos sin desembolso inicial.' },
    ...
    { icon: FileText, title: 'Un solo contrato para todo', desc: 'Sin necesidad de créditos adicionales ni préstamos personales para cubrir los gastos.' },
  ...
  const requisitos = [
    ...
    'Perfil de solvencia sólido (la financiación total requiere mayor garantía)',
    'No es necesario tener ahorros para entrada ni gastos',
  ];

  const faqs = [
    {
      question: '¿Qué gastos exactamente cubre la Hipoteca 100% + Gastos?',
      answer: 'Además del 100% del precio de compraventa, esta modalidad puede cubrir los gastos asociados a la compra: Impuesto de Transmisiones Patrimoniales (ITP) o IVA según el caso, notaría, registro de la propiedad y gestoría. En total, estos gastos suelen representar entre un 8% y un 12% adicional sobre el precio de compra.',
    },
    {
      question: '¿Es más difícil conseguir este tipo de hipoteca que una convencional?',
      answer: 'Sí, al financiar más del 100% del precio de compra, los bancos son más exigentes con el perfil del solicitante. Se requiere un nivel de ingresos más holgado y un historial crediticio impecable. Por eso es fundamental contar con un bróker hipotecario como Avanza Hipotecas que sepa a qué entidades presentar cada perfil.',
    },
    {
      question: '¿Qué perfil de cliente tiene más posibilidades de conseguirla?',
      answer: 'Los perfiles con mayor probabilidad de éxito son trabajadores con contrato indefinido, antigüedad laboral consolidada, ingresos netos suficientes para que la cuota no supere el 35% de los ingresos mensuales y sin deudas activas. Los funcionarios y empleados con nómina alta también tienen muy buenas opciones.',
    },
    {
      question: '¿El tipo de interés es más alto al financiar por encima del 80%?',
      answer: 'En algunos casos puede haber un diferencial ligeramente superior, ya que el riesgo para el banco es mayor. Sin embargo, al comparar más de 20 entidades, encontramos las condiciones más competitivas del mercado para tu perfil concreto. El ahorro en tiempo y negociación que obtienes con nuestro servicio supera con creces cualquier diferencia.',
    },
  ];
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              ...
              >
                Financia el precio de tu vivienda{' '}
                <span className="text-[#2EBFA5]">y todos los gastos de compra</span>
              </motion.h1>
              <motion.p
                initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
                className="text-xl text-gray-600 mb-8 leading-relaxed"
              >
                Cubre el 100% del precio de compraventa más notaría, registro e impuestos en un solo préstamo.
              </motion.p>
              <motion.div
            ...
            >
              <div className="relative">
                <img src="https://images.unsplash.com/photo-1554224155-6726b3ff858f?w=800&q=80" alt="Persona firmando documentos de compraventa" className="rounded-3xl shadow-2xl w-full object-cover h-[480px]" />
                <div className="absolute -bottom-6 -left-6 bg-white p-4 rounded-2xl shadow-lg flex items-center space-x-3">
                  ...
                  <div>
                    <p className="font-bold text-[#1A3C40]">100% + Gastos</p>
                    <p className="text-sm text-gray-600">Sin desembolso inicial</p>
                  </div>
                </div>
              </div>
            </motion.div>
      ...
      <section className="py-20 bg-[#1A3C40] text-white">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
            ...
            <p className="text-xl text-gray-300 max-w-2xl mx-auto">Para quienes quieren comprar su vivienda sin necesidad de ningún desembolso inicial</p>
          </motion.div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
            {[
              { icon: Users, title: 'Sin ahorros para entrada ni gastos', desc: 'Compradores sin liquidez que quieren acceder a la compra con ingresos estables' },
              { icon: Home, title: 'Primera vivienda o cambio de residencia', desc: 'Ideal para quien compra por primera vez o cambia de hogar' },
              { icon: FileText, title: 'Perfil laboral consolidado', desc: 'Trabajadores con contrato indefinido, funcionarios o autónomos con trayectoria sólida' },
              { icon: Shield, title: 'Quienes preservan sus ahorros', desc: 'También para quienes tienen ahorros pero prefieren mantenerlos para otras necesidades' },
            ].map((item, i) => (
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
            <h2 className="text-4xl md:text-5xl font-bold text-[#1A3C40] mb-4">Ventajas de la <span className="text-[#2EBFA5]">Hipoteca 100% + Gastos</span></h2>
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">Tu vivienda completamente financiada, sin desembolso inicial de ningún tipo</p>
          </motion.div>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
              ...
              <p className="text-xl text-gray-600">Comprueba si tu perfil encaja con la Hipoteca 100% + Gastos</p>
            </motion.div>
      ...
      {/* ── FORMULARIO ── */}
      <section id="contacto-h100g" className="py-20 bg-[#E9ECEF]">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
            ...
            <p className="text-xl text-gray-700 max-w-3xl mx-auto">Completa el formulario y un experto te contactará en menos de 24 horas</p>
          </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-white rounded-2xl p-8 shadow-sm">
              ...
              <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <Textarea id="message" name="message" value={formData.message} onChange={handleInputChange} className="mt-2" placeholder="Precio de la vivienda, ingresos aproximados, situación laboral..." />
                </div>
              ...
              <div className="bg-white rounded-2xl p-8 shadow-sm">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    { n: 2, title: 'Búsqueda entre +20 bancos', desc: 'Comparamos las entidades que ofrecen financiación total incluidos gastos' },
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <Link to="/hipoteca-100" className="bg-white rounded-2xl p-5 flex items-start gap-4 hover:shadow-md transition-shadow border border-gray-100 hover:border-[#2EBFA5]/40 group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">Si solo necesitas financiar el precio de compra sin entrada, sin cubrir los gastos de compraventa</p>
                </div>
              ...
              <Link to="/hipoteca-joven" className="bg-white rounded-2xl p-5 flex items-start gap-4 hover:shadow-md transition-shadow border border-gray-100 hover:border-[#2EBFA5]/40 group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">¿Tienes menos de 35 años? Condiciones exclusivas, aval del Estado y mayor financiación para jóvenes compradores</p>
                </div>
                <ArrowRight className="w-4 h-4 text-gray-400 flex-shrink-0 group-hover:text-[#2EBFA5] transition-colors" />
              </Link>
            </div>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
            ...
            <p className="text-xl text-gray-600">Todo lo que necesitas saber sobre la Hipoteca 100% + Gastos</p>
          </motion.div>
          <div className="max-w-4xl mx-auto">
            {faqs.map((faq, i) => (
...
export default Hipoteca100GastosPage;
